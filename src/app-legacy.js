...
let lineSelection = { file: null, startLine: null, endLine: null, side: "RIGHT" };
...
let scrollTargetAfterRender = null; // element ID to scroll to after next render
...
async function postCommentRange(path, startLine, endLine, side, body) {
  if (!isGitHubPR()) return;
  const { owner, repo, number } = data.meta;
...
function renderCommentComposer(filePath) {
  const fileName = filePath.split("/").pop();
  return `
    <details class="comment-composer" data-file="${esc(filePath)}">
      <summary class="comment-composer-toggle">Leave a review comment on ${esc(fileName)}…</summary>
      <div class="comment-composer-body">
        <textarea class="comment-textarea" placeholder="Write your comment..." rows="2" data-comment-file="${esc(filePath)}"></textarea>
        <div class="comment-actions">
          <input type="number" class="comment-line-input" placeholder="Start line" min="1" data-comment-start-line="${esc(filePath)}" />
          <input type="number" class="comment-line-input" placeholder="End line" min="1" data-comment-end-line="${esc(filePath)}" />
          <select class="comment-side-select" data-comment-side="${esc(filePath)}">
            <option value="RIGHT" selected>New code</option>
...
function getProgress() {
  if (!data?.walkthrough?.sections) return { reviewed: 0, total: 0, pct: 0, hunksReviewed: 0, hunksTotal: 0 };
  ...
  // Count hunk-level (file block) progress
  let hunksTotal = 0;
  let hunksReviewed = 0;
  for (const s of data.walkthrough.sections) {
    ...
    for (const f of fileGroups) {
      hunksTotal++;
      if (reviewState[`hunk:${s.id}:${f}`]?.reviewed) hunksReviewed++;
    }
  }
...
function render() {
  ...
  renderMermaid(app);
  if (window.hljs) {
    app.querySelectorAll(".comment-body pre code, .hunk-annotation pre code").forEach((el) => {
  ...
  if (viewMode !== "focus") {
    if (scrollTargetAfterRender) {
      const target = document.getElementById(scrollTargetAfterRender);
  ...
  return `
    <header class="page-header">
      ...
      <div class="meta">
        ...
        <span class="meta-item">${progress.hunksReviewed}/${progress.hunksTotal} files reviewed</span>
        ${reviewers}
      </div>
    </header>
  `;
...
function renderToolbar() {
  // Old toolbar hidden — replaced by bottom bar + command palette
  return "";
}
...
function renderBottomBar() {
  ...
  return `
    <div class="bottom-bar">
      <div class="bar-seg bar-green"><div class="bar-progress"><div class="bar-progress-fill" style="width:${pctWidth}%"></div></div> ${progress.hunksReviewed}/${progress.hunksTotal}</div>
      <div class="bar-seg">${progress.reviewed}/${progress.total} sec</div>
      <div class="bar-seg ${diffViewMode === "side-by-side" ? "bar-on" : ""} bar-clickable" data-mode="side-by-side">Split</div>
      <div class="bar-seg ${diffViewMode === "unified" ? "bar-on" : ""} bar-clickable" data-mode="unified">Unified</div>
      <div class="bar-seg bar-clickable" id="btn-toggle-comments">💬 ${commentCount}</div>
      <div class="bar-right">
        ${gh ? `<div class="bar-seg bar-green bar-clickable" id="btn-approve">✓ Approve</div>` : ""}
        ${gh ? `<div class="bar-seg bar-red bar-clickable" id="btn-request-changes">✕ Changes</div>` : ""}
        <div class="bar-seg bar-clickable" id="btn-action-panel"><kbd style="font-family:var(--mono);font-size:10px;padding:1px 5px;background:var(--bg);border:1px solid var(--border-light);border-radius:3px;">.</kbd> Actions</div>
      </div>
    </div>
  `;
}
...
function renderActionPanel() {
  ...
  for (const [group, groupItems] of byGroup) {
    groupsHtml += `<div class="action-panel-group">
      <div class="action-panel-group-label">${esc(group)}</div>
      ${groupItems.map((item, i) => `
        <div class="action-panel-item" data-action-key="${esc(item.key || "")}">
          <kbd>${esc(item.key)}</kbd>
          <span class="action-panel-item-label ${item.active ? "active-state" : ""}">${esc(item.label)}</span>
          ${item.active ? '<span class="action-panel-item-check">✓</span>' : ""}
        </div>
      `).join("")}
    </div>`;
  }
  ...
  return `
    <div class="action-panel-backdrop" id="action-panel-backdrop"></div>
    <div class="action-panel" id="action-panel">
      <div class="action-panel-inner">${groupsHtml}</div>
    </div>
  `;
}
...
function getActionItems() {
  ...
  const commentCount = data?.comments?.length || 0;
  return [
    { group: "Display", label: "Split diff", key: "s", action: () => { diffViewMode = "side-by-side"; render(); }, active: diffViewMode === "side-by-side" },
    { group: "Display", label: "Unified diff", key: "u", action: () => { diffViewMode = "unified"; render(); }, active: diffViewMode === "unified" },
    { group: "Display", label: "Dark mode", key: "d", action: () => { darkMode = !darkMode; localStorage.setItem("review-tool-dark", String(darkMode)); render(); } },
    { group: "Display", label: `Comments (${commentCount})`, key: "c", action: () => { showComments = !showComments; render(); }, active: showComments },
    { group: "Display", label: "Hide reviewed", key: "h", action: () => { hideReviewed = !hideReviewed; render(); }, active: hideReviewed },
    { group: "Sections", label: "Expand all", key: "e", action: () => { collapsedSections.clear(); collapsedHunks.clear(); saveReviewState(); render(); } },
    { group: "Sections", label: "Collapse all", key: "w", action: () => { data.walkthrough.sections.forEach((s) => collapsedSections.add(s.id)); saveReviewState(); render(); } },
    { group: "Navigate", label: "Next unreviewed", key: "n", action: () => { closeActionPanel(); navigateToUnreviewed(); } },
    { group: "Navigate", label: "Next section", key: "j", action: () => { closeActionPanel(); navigateSection(1); } },
    { group: "Navigate", label: "Prev section", key: "k", action: () => { closeActionPanel(); navigateSection(-1); } },
    { group: "Navigate", label: "Review current", key: "r", action: () => { closeActionPanel(); reviewCurrentSection(); } },
    ...(gh ? [
      { group: "Review", label: "Approve PR", key: "a", action: () => { closeActionPanel(); openReviewModal("APPROVE", "Approve this PR"); } },
      { group: "Review", label: "Request changes", key: "x", action: () => { closeActionPanel(); openReviewModal("REQUEST_CHANGES", "Request Changes"); } },
      { group: "Review", label: "Refresh comments", key: "f", action: () => { closeActionPanel(); refreshComments(); } },
    ] : []),
    ...(data?.meta?.url ? [
      { group: "Navigate", label: "Open PR on GitHub", key: "g", action: () => { closeActionPanel(); window.open(data.meta.url, "_blank"); } },
    ] : []),
    { group: "Review", label: "Reset progress", key: "0", action: () => { closeActionPanel(); if (confirm("Reset all review progress?")) { reviewState = {}; saveReviewState(); render(); } } },
    { group: "View", label: "Editorial", key: "1", action: () => { viewMode = "editorial"; localStorage.setItem("review-tool-view", viewMode); render(); }, active: viewMode === "editorial" },
    { group: "View", label: "Sidebar", key: "2", action: () => { viewMode = "sidebar"; localStorage.setItem("review-tool-view", viewMode); render(); }, active: viewMode === "sidebar" },
    { group: "View", label: "Focus", key: "3", action: () => { viewMode = "focus"; localStorage.setItem("review-tool-view", viewMode); render(); }, active: viewMode === "focus" },
    { group: "View", label: "Split", key: "4", action: () => { viewMode = "split"; localStorage.setItem("review-tool-view", viewMode); render(); }, active: viewMode === "split" },
    { group: "View", label: "Developer", key: "5", action: () => { viewMode = "developer"; localStorage.setItem("review-tool-view", viewMode); render(); }, active: viewMode === "developer" },
    { group: "View", label: "Dashboard", key: "6", action: () => { viewMode = "dashboard"; localStorage.setItem("review-tool-view", viewMode); render(); }, active: viewMode === "dashboard" },
    { group: "Help", label: "Shortcuts", key: "?", action: () => { closeActionPanel(); toggleShortcutsModal(); } },
  ];
}
...
function toggleActionPanel() {
  if (actionPanelOpen) closeActionPanel();
...
function renderSection(section, index) {
  ...
  if (hideReviewed && reviewed) return "";
...
function renderFileBlocksWithExpand(displayFile, mode, filePath, hunkKey, fileHunks) {
  const blocks = displayFile.blocks;
  if (!blocks.length) return renderFileDiff(displayFile, mode);
  const canExpand = isGitHubPR();
  let html = "";

  // Track which annotations have been rendered
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // Insert annotations that reference lines within this block
    if (fileHunks) {
      const { lastNew } = getBlockEndLines(block);
      const blockStart = block.newStartLine;
      const blockEnd = lastNew;
      for (let j = 0; j < fileHunks.length; j++) {
        if (renderedAnnotations.has(j)) continue;
        const hunk = fileHunks[j];
        if (!hunk.annotation) continue;
        const hStart = hunk.startLine || 0;
        const hEnd = hunk.endLine || hStart;
        // Annotation belongs to this block if its lines overlap
        if (hStart && hEnd && hStart <= blockEnd && hEnd >= blockStart) {
          const imp = hunk.importance || "important";
          html += `<div class="hunk-annotation annotation-${imp}">
            <span class="annotation-lines">${hStart === hEnd ? `L${hStart}` : `L${hStart}\u2013${hEnd}`}</span>
            ${md(hunk.annotation)}
          </div>`;
          renderedAnnotations.add(j);
        }
      }
    }
  ...
  // Render any remaining annotations that didn't match a block (fallback)
  if (fileHunks) {
    for (let j = 0; j < fileHunks.length; j++) {
      if (renderedAnnotations.has(j)) continue;
      const hunk = fileHunks[j];
...
function renderGroupedHunks(hunks, sectionId) {
  ...
  for (const [filePath, fileHunks] of fileGroups) {
    ...
    const hunkReviewed = reviewState[`hunk:${hunkKey}`]?.reviewed;

    if (hideReviewed && hunkReviewed) continue;
    ...
    const fileAge = renderFileAgeBadge(filePath);
    const fileChurn = renderFileChurnBadge(filePath);

    html += `
      <div class="hunk-group importance-${topImportance} ${hunkReviewed ? "hunk-reviewed" : ""}">
        <div class="hunk-header" data-hunk-toggle="${esc(hunkKey)}">
          <label class="hunk-review-check" onclick="event.stopPropagation()" title="Mark file as reviewed">
            <input type="checkbox" ${hunkReviewed ? "checked" : ""} data-hunk-review="${esc(hunkKey)}" />
          </label>
    ...
    if (!isCollapsed) {
      // Show the file diff with annotations interleaved before their matching blocks
      if (file) {
        ...
        html += renderFileBlocksWithExpand(displayFile, diffViewMode, filePath, hunkKey, fileHunks);
        html += `</div>`;
      } else {
      ...
      if (isGitHubPR() && showComments) {
        html += renderCommentComposer(filePath);
      }
    }
      ...
      <div class="comment-thread">
        <div class="comment">
          <div class="comment-meta">
            ...
            ${thread.line ? `<span class="comment-line">${thread.startLine && thread.startLine !== thread.line ? `Lines ${thread.startLine}\u2013${thread.line}` : `Line ${thread.line}`}</span>` : ""}
    ...
    for (const [dir, files] of groups) {
      ...
      if (!groupCollapsed) {
        for (const filePath of files) {
          ...
          if (!fileCollapsed) {
            if (file) {
              html += `<div class="hunk-diff" data-hunk-file="${esc(filePath)}">${renderFileDiff(file, diffViewMode)}</div>`;
            }
...
function renderFileAgeBadge(filePath) {
  const age = data?.gitHistory?.fileAges?.[filePath];
  if (!age) return "";
  const days = age.daysSince;
  let label, cls;
  if (days > 365) {
    label = `${Math.floor(days / 365)}y old`;
    cls = "age-old";
  } else if (days > 90) {
  ...
  return `<span class="file-age-badge ${cls}" title="Last modified ${age.lastModified?.split('T')[0]} by ${esc(age.lastAuthor || 'unknown')}">${label}</span>`;
}

function renderFileChurnBadge(filePath) {
  const churn = data?.gitHistory?.churn?.[filePath];
  if (!churn || churn.touchCount < 2) return "";
  return `<span class="file-churn-badge" title="Touched ${churn.touchCount} times across commits in this PR">${churn.touchCount}× revised</span>`;
}

function renderCommitTimeline() {
  const commits = data?.gitHistory?.commits;
...
function highlightSelectedLines() {
  ...
  if (!lineSelection.file || lineSelection.startLine === null) return;

  const hunkDiffs = document.querySelectorAll(`.hunk-diff[data-hunk-file="${CSS.escape(lineSelection.file)}"]`);
  for (const hunkDiff of hunkDiffs) {
    const cells = hunkDiff.querySelectorAll(".d2h-code-side-linenumber, .d2h-code-linenumber");
    for (const cell of cells) {
      const num = parseInt(cell.textContent.trim());
      if (!num) continue;
      if (num >= lineSelection.startLine && num <= lineSelection.endLine) {
        const row = cell.closest("tr");
...
function populateComposerFromSelection() {
  if (!lineSelection.file) return;

  const composer = document.querySelector(`.comment-composer[data-file="${CSS.escape(lineSelection.file)}"]`);
  if (!composer) return;

  // Auto-expand the collapsed composer
  if (!composer.open) composer.open = true;

  const startInput = composer.querySelector(`[data-comment-start-line]`);
  const endInput = composer.querySelector(`[data-comment-end-line]`);
  const sideSelect = composer.querySelector(`[data-comment-side]`);
  const textarea = composer.querySelector(".comment-textarea");

  if (startInput) startInput.value = lineSelection.startLine;
  if (endInput) endInput.value = lineSelection.endLine;
  if (sideSelect) sideSelect.value = lineSelection.side;
...
function setupHandlers() {
  ...
  // Action panel backdrop close
  document.getElementById("action-panel-backdrop")?.addEventListener("click", () => closeActionPanel());

  // Action panel item clicks
  document.querySelectorAll(".action-panel-item").forEach((el) => {
  ...
  document.querySelectorAll("[data-section-review]").forEach((cb) => {
    cb.addEventListener("change", () => {
      ...
      if (cb.checked) {
        ...
        const sectionElId = id === "__remaining" ? "section-remaining" : `section-${id}`;
        scrollTargetAfterRender = sectionElId;
      }
      saveReviewState();
      render();
    });
  ...
  // Hunk-level review checkboxes
  document.querySelectorAll("[data-hunk-review]").forEach((cb) => {
    cb.addEventListener("change", () => {
      const key = cb.dataset.hunkReview;
      reviewState[`hunk:${key}`] = {
        reviewed: cb.checked,
        timestamp: new Date().toISOString(),
      };
      saveReviewState();
  ...
  // These actions now live in the command palette (getCommandItems)
  ...
  document.querySelectorAll("[data-post-comment]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const textarea = document.querySelector(`[data-comment-file="${CSS.escape(filePath)}"]`);
      const startLineInput = document.querySelector(`[data-comment-start-line="${CSS.escape(filePath)}"]`);
      const endLineInput = document.querySelector(`[data-comment-end-line="${CSS.escape(filePath)}"]`);
      const sideSelect = document.querySelector(`[data-comment-side="${CSS.escape(filePath)}"]`);
      const body = textarea?.value?.trim();
      const startLine = startLineInput?.value ? parseInt(startLineInput.value) : null;
      const endLine = endLineInput?.value ? parseInt(endLineInput.value) : null;
      const side = sideSelect?.value || "RIGHT";

      if (!body) return;

      const effectiveEndLine = endLine || startLine || 1;
      const effectiveStartLine = startLine || effectiveEndLine;
      ...
      try {
        if (effectiveStartLine !== effectiveEndLine) {
          await postCommentRange(filePath, effectiveStartLine, effectiveEndLine, side, body);
        } else {
          await postComment(filePath, effectiveEndLine, side, body);
        }
        textarea.value = "";
        if (startLineInput) startLineInput.value = "";
        if (endLineInput) endLineInput.value = "";
        lineSelection = { file: null, startLine: null, endLine: null, side: "RIGHT" };
        render();
      } catch (err) {
  ...
  document.getElementById("btn-action-panel")?.addEventListener("click", () => toggleActionPanel());
  ...
  // Click-to-select line numbers for commenting
  if (!window.__lineSelectBound) {
    document.addEventListener("click", (e) => {
      if (e.ctrlKey || e.metaKey) return;

      // Match both side-by-side and unified line number cells
      const lineNumCell = e.target.closest(".d2h-code-side-linenumber, .d2h-code-linenumber");
      if (!lineNumCell) return;

      const lineNum = parseInt(lineNumCell.textContent.trim());
      if (!lineNum) return;

      // Find the file path from the closest hunk-diff container
      const hunkDiff = lineNumCell.closest(".hunk-diff");
      if (!hunkDiff) return;
      const filePath = hunkDiff.dataset.hunkFile;
      if (!filePath) return;

      // Determine side: LEFT for old-side (delete side), RIGHT for new-side
      let side = "RIGHT";
      const row = lineNumCell.closest("tr");
      if (row) {
        const cells = [...row.querySelectorAll(".d2h-code-side-linenumber")];
        if (cells.length === 2 && lineNumCell === cells[0]) {
          side = "LEFT";
        }
        // Unified mode: check if this is a delete line
        if (lineNumCell.classList.contains("d2h-code-linenumber")) {
          const codeLine = row.querySelector(".d2h-code-line-ctn");
          if (codeLine?.closest(".d2h-del")) side = "LEFT";
        }
      }

      e.preventDefault();

      if (e.shiftKey && lineSelection.file === filePath && lineSelection.startLine !== null) {
        // Shift+click: set end of range
        const start = Math.min(lineSelection.startLine, lineNum);
        const end = Math.max(lineSelection.startLine, lineNum);
        lineSelection.endLine = end;
        lineSelection.startLine = start;
      } else {
        // Regular click: start new selection
        lineSelection = { file: filePath, startLine: lineNum, endLine: lineNum, side };
      }

      highlightSelectedLines();
      populateComposerFromSelection();
    });
    window.__lineSelectBound = true;
  }
}
